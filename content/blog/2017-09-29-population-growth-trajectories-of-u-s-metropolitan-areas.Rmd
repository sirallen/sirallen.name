...
```
```{r, eval=FALSE}
# Grab 2010-16 MSA-level estimates from the API
msa10.16 =
  GET(
...
# Likewise for 2000-09 county-level estimates
county00.09 =
  GET(
...
$$
\min_\beta \left\{ (y - X\beta)^\intercal (y - X\beta) + \gamma ||(X \beta)'(2016) - (1 + g_{2015, 2016})|| \right\}
$$
...
(2) For kicks, here's a joyplot of growth densities by year, for all MSAs with observations over the entire sample period (which includes all but about 40 of them). Tails are truncated by the `rel_min_height` argument. It actually looks like median growth has slowed somewhat, contrary to my initial belief.
```{r, warning=FALSE}
library(ggridges)

msa03.16[, .(YEAR=YEAR[-1], g=100*diff(log(POP_Idx))), by='MSA'] %>%
  ggplot(aes(x = g, y = factor(YEAR, levels=2016:2004))) +
