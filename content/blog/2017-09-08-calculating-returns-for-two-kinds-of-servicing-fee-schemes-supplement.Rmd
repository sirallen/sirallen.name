...
R1 = Vectorize(
  function(s, P0, r, m) {
    ...
    # P0, r, m -- args to A()
    A_ = A(P0, r, m)
    R = (sum(A_ - s * P_i(A_, P0, r, 1:m)) - P0) / P0
    # annualize
    (1 + R)^(12/m) - 1
  },
  vectorize.args = 'm')
...
R1 = Vectorize(
  function(s, P0, r, m) {
    ...
    R = (sum(A_seq) - s * sum(P_i(A_seq, P0, r, 1:m)) - P0) / P0
    # annualize
    (1 + R)^(12/m) - 1
  },
  vectorize.args = 'm')
...
With this more realistic payment sequence, an investor would still prefer early payment under $R_1$, though the "penalty" on ROI of one additional month---corresponding to the slope of the curve---is not as pronounced for small $m$. Interestingly, the curve corresponding to $R_2$ attains an interior maximum, suggesting that a Note investor might be best off if the borrower prepays, but not _too_ early.
