...
The features of this model can be studied via a simulation exercise, in the spirit of agent-based modeling. One challenging problem (which I won't attempt in this post) is to figure out the beliefs and utility functions which result in an admissions outcome that is consistent with $\{I_k\}$, in an equilibrium sense.

I've made the code available on my [GitHub](https://github.com/sirallen/college-admissions-toy-model).
...
with $(\underline{r}, \overline{r}) = (.05, 1)$, and the utility function (common to all applicants) has the form
