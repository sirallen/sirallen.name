---
title: Optimal Payment Sequence
...
_This post is an extension of [Calculating Returns for Two Kinds of Servicing Fee Schemes](https://sirallen.name/blog/calculating-returns-for-two-kinds-of-servicing-fee-schemes-supplement/)._

In the previous post I explored the question of why, given one of two servicing fee schemes, an investor in Lending Club loans may or may not desire early payment by the borrower. I assumed the payment sequence $\{A_i\}$ to make my case, but it's easy to imagine the more general question of, What is the best $\{A_i\}$ an investor could hope for, given $m$? From an investing standpoint, this isn't really a useful question to ask---after all, investors don't get to choose how the borrower repays. But it sounds like it might be an interesting math problem, so why not?

Formally, given $(P_0, r, m, T)$ we'd like to know what sequence of payments $\{A_i\}_{i=1}^m$ maximizes the investor's IRR, where principal amortizes according to
$$
...
Let me first introduce the notation $dX/dA_{i \rightarrow j}$ to describe the marginal change in $X$ from moving part of $A_i$ to $A_j$, holding all other payments fixed. The claim, then, is that $dR_1/dA_{1 \rightarrow j} \leq 0$ for all $1 < j \leq m$.

Importantly, $A_{i \rightarrow j}$ cannot be 1:1 due to both compounding and the constraint $P_m = 0$. In particular, while $dA_i/dA_{i \rightarrow j} = -1$,
$$
